...
static float angle_threshold = 0.1;
...
int main() {
	...
	read_file("../../A3_Normalenvektoren/shaders/gouraud_vert.spv", &vertex_byte_code, &vertex_code_size);
	uint8_t * fragment_byte_code = 0;
	int fragment_code_size;
	read_file("../../A3_Normalenvektoren/shaders/gouraud_frag.spv", &fragment_byte_code, &fragment_code_size);
	...
	settings_data.light_pos = glm::vec4(100, 0, 0, 1);
	...
	while (!glfwWindowShouldClose(window)) {
...

		{
			...
			vkal_draw(image_id, pipeline_filled_polys, g_model.offset, g_model.vertex_count);
			if (draw_normals) {
